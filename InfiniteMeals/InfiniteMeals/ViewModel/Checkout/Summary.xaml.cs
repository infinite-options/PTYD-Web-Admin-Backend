...
namespace InfiniteMeals.ViewModel.Checkout {
    ...
    public partial class Summary : ContentPage {
        ...
        private async void ConfirmClicked(object sender, EventArgs e) {
            ...
            string userPassword = await DisplayPromptAsync("Password", "Please enter your password");
        ...
        private async void checkout() { // completes the checkout and sends the information as a json to the database
            ...
            try {
                ...
                var content = await client.GetStringAsync(accountSaltURL + App.Database.GetLastItem().Email); // get athe requested account salt
                ...
                byte[] data = sHA512.ComputeHash(Encoding.UTF8.GetBytes(password + obj.result[0].passwordSalt)); // take the password and account salt to generate hash
